...
def get_categorical_real_columns(data):
    categorical_columns_indices = []
    real_columns_indices = []
    ...
    for i in range(len(data[0])):
        ...
        if is_categorical(is_int_arr):
            ...
            real_columns_indices.append(i)

    return categorical_columns_indices, real_columns_indices


"""Check if the array is full of True"""
...
def main(args: argparse.Namespace):
    ...
    categorical_columns_indices, real_columns_indices = get_categorical_real_columns(train_data)

    ohe = ColumnTransformer([("cat", sklearn.preprocessing.OneHotEncoder(
        sparse=False, handle_unknown="ignore"), categorical_columns_indices)])


    ss = ColumnTransformer(transformers=[("norm", sklearn.preprocessing.StandardScaler(), real_columns_indices)])
    feat = sklearn.preprocessing.PolynomialFeatures(2, include_bias=False)

    ohe.fit(train_data)
    ss.fit(train_data)

    categorised = ohe.transform(train_data)
    scaled = ss.transform(train_data)

    train_data = np.concatenate((categorised, scaled), axis=1)



    if len(real_columns_indices) != 0:
        feat.fit(train_data)
        train_data = feat.transform(train_data)
        categorised_test = ohe.transform(test_data)
        scaled_test = ss.transform(test_data)

        test_data = np.concatenate((categorised_test, scaled_test), axis=1)
        test_data = feat.transform(test_data)

    else:
        train_data = categorised
        feat.fit(train_data)
        train_data = feat.transform(train_data)

        categorised_test = ohe.transform(test_data)
        test_data = feat.transform(categorised_test)
