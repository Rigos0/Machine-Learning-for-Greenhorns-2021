...
def main(args: argparse.Namespace) -> float:
    ...
    pipe = sklearn.pipeline.Pipeline(steps=[('minmax', MinMaxScaler()),
                                            ('features', PolynomialFeatures()),
                                            ("logistic_regression", LogisticRegression(random_state=args.seed))])

    param_grid = [{'features__degree': [1, 2],
                   'logistic_regression__C': [0.01, 1, 100],
                   'logistic_regression__solver': ['lbfgs', 'sag']}]

    search = sklearn.model_selection.GridSearchCV(pipe, param_grid, cv=sklearn.model_selection.StratifiedKFold(5))
