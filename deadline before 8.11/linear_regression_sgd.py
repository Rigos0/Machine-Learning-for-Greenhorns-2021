...
def get_rmse(predictions, targets):
    return sklearn.metrics.mean_squared_error(predictions, targets, squared=False)
...
def main(args: argparse.Namespace):
    ...
    train_data, test_data, train_target, test_target = \
        sklearn.model_selection.train_test_split(data, target,
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(len(permutation)):
            index = permutation[i]
            next_data_point = train_data[index]
            ten_counter += 1

            gradient = (np.dot(next_data_point, weights) - train_target[index]) * next_data_point

            all_gradient += gradient
            if ten_counter == args.batch_size:
                ten_counter = 0
                all_gradient /= args.batch_size
                weights = weights - args.learning_rate * (all_gradient + args.l2 * weights)
                all_gradient = 0

        predictions = np.matmul(train_data, weights)

        rmse = get_rmse(predictions, train_target)
        train_rmses.append(rmse)

        test_pred = np.matmul(test_data, weights)
        rmse = get_rmse(test_pred, test_target)
    ...
    explicit_rmse = get_rmse(predictions, test_target)
